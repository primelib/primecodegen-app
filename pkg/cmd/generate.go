...
func generateLocal(dir string) {
	...
	// load config
	config, err := primelib.ConfigFromString(string(bytes))
	if err != nil {
	...
	for _, module := range config.Modules {
		genErr := primelib.Execute(dir, module, api.Repository{})
		if genErr != nil {
			log.Warn().Err(genErr).Str("module", module.Name).Msg("failed to generate code")
		}
	}
}
